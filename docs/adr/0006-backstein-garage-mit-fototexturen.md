...
- Wände innen und außen, Boden und Vorplatz sowie die Werkbankplatte bekommen kachelnde
  Fototexturen: `Bricks059`, `Asphalt033` und `Wood092` von ambientCG (CC0), als Farbe 2k
  (Asphalt 1k, entsättigt) und Normal Map 1k in `blender/textures/`. Alles andere bleibt
  Flächenfarbe, die Geometrie bleibt Low-Poly. Schränke und Schubladenfront werden
  `Lack_Gruen`, die Sockelleiste und die Bodenfugen entfallen, das Tor bekommt acht
  Kassetten auf dem Außensegment, neben dem Tor hängen zwei Außenleuchten.
- UVs in Weltmetern: `garage_lib.box_project_uvs` projiziert jede Fläche über ihre
  ...
  angibt (Backstein 1,05 m, Asphalt 2,5 m, Holz 0,8 m). Ein Ziegel ist damit überall gleich
...
- Texturen reisen als WebP im GLB (`EXT_texture_webp`, Blender-Exporter, Qualität 90), nicht
  ...
  (`lib/garage/capability.ts`). GLB 2,0 MB, davon 1,8 MB Texturen, unter dem Budget von 3 MB.
...
- `blender/textures/` liegt mit 4,6 MB im Repo. Die Quelle ist die `.blend` plus die
